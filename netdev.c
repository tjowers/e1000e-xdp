...
static irqreturn_t e1000_intr_msi(int __always_unused irq, void *data)
{
	...
	printk("[E1000E]: using e1000_intr_msi\n");
...
static irqreturn_t e1000_intr(int __always_unused irq, void *data)
{
	...
	printk("[E1000E]: using e1000_intr");
...
#else
// ensure dma region is set up properly so that interrupts are generated
	// Run XDP Program Here
/*	if (adapter->xdp_prog) {
		rcu_read_lock();
		bpf_prog_run_xdp(adapter->xdp_prog, &xdp);
		rcu_read_unlock();
	}*/
#endif
...
void e1000e_set_interrupt_capability(struct e1000_adapter *adapter)
{
		...
		if (adapter->flags & FLAG_HAS_MSIX) {
			...
			adapter->msix_entries = kcalloc(adapter->num_vectors,
							sizeof(struct
							       msix_entry),
							GFP_KERNEL);
			printk("[E1000E]: Interrupt Mode -> MSI-X\n");
			if (adapter->msix_entries) {
	...
	case E1000E_INT_MODE_MSI:
		printk("[E1000E]: Interrupt Mode -> MSI\n");
...
static int e1000_request_msix(struct e1000_adapter *adapter)
{
	...
	printk("MSIX Requesting Interrupt\n");
...
static int e1000e_poll(struct napi_struct *napi, int budget)
{
	...
	int xdp_rv, tx_cleaned = 1, work_done = 0;
	...
	if (adapter->xdp_prog) {
		printk("STARTING XDP\n");
		rcu_read_lock();
		xdp_rv = bpf_prog_run_xdp(adapter->xdp_prog, NULL);
		rcu_read_unlock();
		printk("FINISHING XDP: returned --> %d\n", xdp_rv);
	}
...
static irqreturn_t e1000_intr_msix(int __always_unused irq, void *data)
{
	...
	printk("[E1000E]: using e1000_intr_msix\n");
...
static const struct net_device_ops e1000e_netdev_ops = {
	...
	// XDP_NETDEVICE
	.ndo_bpf 		= e1000e_bpf,
	.ndo_xsk_wakeup 	= e1000e_xsk_wakeup,
